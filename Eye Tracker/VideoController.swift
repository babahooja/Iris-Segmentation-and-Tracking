...
//  AV Foundation
//
//  Created by Himanshu Ahuja on 17/05/18.
...
extension VideoController {
	func prepare(completionHandler: @escaping (Error?) -> Void) {
		...
		func configureCaptureDevices() throws {
			...
			// adds the devices array to the session
			let cameras = session.devices
			if cameras.isEmpty { throw VideoControllerError.noCamerasAvailable }
			
			for camera in cameras {
				// if the device position is .front, set the front camera property
				// to that camera
		...
		func configureDeviceInputs() throws {
			...
			if let rearCamera = self.rearCamera {
				...
				else { throw VideoControllerError.inputsAreInvalid }
				
				self.currentCameraPosition = .front
			}
				
			else { throw VideoControllerError.noCamerasAvailable }
		}
	...
	func displayPreview(on view: UIView) throws {
		guard let captureSession = self.captureSession, captureSession.isRunning
			else { throw VideoControllerError.captureSessionIsMissing }
...
//		view.layer.insertSublayer(self.previewLayer!, at: 0)
		...
		// bring camera button to front
//		view.bringSubviewToFront(caButton)
		captureSession.startRunning()
	}
	// end of display preview
	
	//	func switchCameraMode() throws {
	//		switch current
	//	}
	//	// end of camera mode switching
	...
	func switchCameras() throws{
		...
		func switchToFrontCamera() throws {
			// get array of all the inputs in the capture session
	...
	// end of switch cameras function
	
	func captureImage(completion: @escaping (UIImage?, Error?) -> Void){
		guard let captureSession = captureSession, captureSession.isRunning
			else {completion(nil, VideoControllerError.captureSessionIsMissing); return}
...
extension VideoController: AVCapturePhotoCaptureDelegate {
	public func photoOutput(_ captureOutput: AVCapturePhotoOutput, didFinishProcessingPhoto photoSampleBuffer: CMSampleBuffer?, previewPhoto previewPhotoSampleBuffer: CMSampleBuffer?,
		...
		if let error = error { self.photoCaptureCompletionBlock?(nil, error) }
			...
			self.photoCaptureCompletionBlock?(nil, VideoControllerError.unknown)
		}
	}
}
